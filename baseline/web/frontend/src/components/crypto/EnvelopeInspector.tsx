...
const fallbackFormats = ["kvlt-binary-v1", "json-v1"];
...
export function EnvelopeInspector({ formatOptions }: Props) {
  ...
  const parse = async () => {
    try { const env = await api.post<EnvelopeSummary>(apiPaths.crypto.inspect, { tenant_id: tenantId, ciphertext: input, source_format: sourceFormat || undefined }); setParsed(env); setTargetFormat(nextTarget(env.format, targetOptions)); setOutput(""); }
    catch (error) { setParsed(null); showToast(`inspect failed: ${(error as Error).message}`, "error"); }
  };
  const convert = async () => {
    try {
      const result = await api.post<{ ciphertext: string; envelope_format: string }>(apiPaths.crypto.convert, { tenant_id: tenantId, ciphertext: input, source_format: sourceFormat || undefined, target_format: targetFormat });
      setOutput(result.ciphertext); showToast(`converted to ${result.envelope_format}`, "success");
  ...
  return <Panel title="Envelope Inspector" action={parsed && <span className="mono" style={{ fontSize: 11, color: "var(--accent)" }}>format: {parsed.format}</span>}>
    <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
      <input className="input" value={input} onChange={(e) => setInput(e.target.value)} placeholder="paste base64 envelope..." />
    ...
    {parsed && <div>
      ...
      {output && <MonoReadout label="Converted envelope (base64)" value={output} copyable />}
    </div>}
  </Panel>;
}
