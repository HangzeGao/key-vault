...
function parseEntries(text: string, policyID: string): ImportKeyReq[] {
  ...
  try {
    ...
    rows = dataLines.map((line) => {
      const [name = "", key_id = "", suite_id = "AES_256_GCM", external_key = "", expires_at = ""] = line.split(",");
      return { name, key_id, suite_id, external_key, expires_at };
    });
  }
  const entries = rows.map((row) => ({
    ...
    policy_id: policyID,
    suite_id: row.suite_id || "AES_256_GCM",
    external_key: row.external_key || "",
    expires_at: row.expires_at || undefined,
  }));
  if (entries.length === 0 || entries.length > 100) throw new Error("import must contain 1-100 entries");
  if (entries.some((entry) => !entry.name.trim() || !entry.external_key.trim())) throw new Error("each entry requires name and external_key");
  return entries;
}
  ...
  return (
    <Modal title="Batch Import Keys" onClose={onClose}>
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div className="batch-import-warning">Data Encryption Keys (DEKs) are sealed in the key plane and never returned. CSV columns: <code>name,key_id,suite_id,external_key,expires_at</code>.</div>
        <textarea className="textarea" value={source} onChange={(event) => setSource(event.target.value)} placeholder='Paste JSON entries or CSV rows. Example: {"name":"orders","suite_id":"AES_256_GCM","external_key":"..."}' style={{ minHeight: 190, fontFamily: '"JetBrains Mono", monospace', fontSize: 11 }} />
